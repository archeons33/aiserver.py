...
if lupa.LUA_VERSION[:2] != (5, 4):
    logger.error(
        f"Please install lupa==1.10. You have lupa {lupa.__version__}.")
...
def new_pretrainedtokenizerbase_from_pretrained(cls, *args, **kwargs):
    tokenizer = old_pretrainedtokenizerbase_from_pretrained(
        cls, *args, **kwargs)
...
# 3: the memory requirement for the model, 4: if the item is a menu or not
# (True/False)
model_menu = {
    'mainmenu': [
        ["Load a model from its directory", "NeoCustom", "", False],
        ["Load an old GPT-2 model (eg CloverEdition)",
                                   "GPT2Custom", "", False],
    ...
    'gptneolist': [
        ...
        ["Pythia 13B (NeoX, Same dataset)",
                      "EleutherAI/pythia-13b", "32GB", False],
        ...
        ["Pythia 800M (NeoX, Same dataset)",
                       "EleutherAI/pythia-800m", "4GB", False],
        ["Pythia 350M (NeoX, Same dataset)",
                       "EleutherAI/pythia-350m", "2GB", False],
...
class vars:
    ...
    # Used when Model ID is an online service, and there is a secondary option
    # for the actual model name
    online_model = ""
    model_selected = ""  # selected model in UI
    # Model Type (Automatically taken from the model config)
    model_type = ""
    ...
    # Whether or not the current RNG seed was specified by the user (in their
    # settings file)
    ...
    # Saved author's note template in settings
    ...
    # List of dictonaries, one dictonary for every action that contains
    # information about the action like alternative options.
    actions_metadata = {}
    ...
    # Raw contents of the model's config.json, or empty dictionary if none
    # found
    modelconfig = {}
    ...
    # Whether or not Lua is running (i.e. wasn't stopped due to an error)
    lua_running = False
    ...
    # Whether or not we are actively checking to see if TPU backend is
    # compiling or not
    checking = False
    ...
    # Filename of soft prompt to load, or an empty string if not using a soft
    # prompt
    spfilename = ""
    userscripts = []     # List of userscripts to load
    # List of previous userscript filenames from the previous time userscripts
    # were send via usstatitems
    last_userscripts = []
    corescript = "default.lua"  # Filename of corescript to load
    # badwords    = []     # Array of str/chr values that should be removed
    # from output
    badwordsids = []
    badwordsids_default = [
    [13460],
    ...
    hascuda = True  # Whether torch has detected CUDA on the system
    usegpu = True  # Whether to launch pipeline with GPU support
    ...
    # Whether or not at least one loaded Lua userscript has a generation
    # modifier
    has_genmod = False
    ...
    # Whether Back button was recently used without Submitting or Retrying
    # after
    recentback = False
    # If a new random game was recently generated without Submitting after,
    # this is the topic used (as a string), otherwise this is None
    recentrng = None
    # If a new random game was recently generated without Submitting after,
    # this is the memory used (as a string), otherwise this is None
    recentrngm = None
    ...
    # Whether the breakmodel option is supported (GPT-Neo/GPT-J/XGLM/OPT only,
    # currently)
    bmsupported = False
    # Something specifically requested Breakmodel to be disabled (For example
    # a models config)
    nobreakmodel = False
    ...
    # Whether we are allowed to use soft prompts (by default enabled if we're
    # using GPT-2, GPT-Neo or GPT-J)
    allowsp = False
    # Embedding dimension of your model (e.g. it's 4096 for GPT-J-6B and 2560
    # for GPT-Neo-2.7B)
    modeldim = -1
    # Filename (without extension) of most recent story JSON file we loaded
    laststory = None
    # Pattern for limiting the output to a single line
    regex_sl = re.compile(r'\n*(?<=.) *\n(.|\n)*')
    # Pattern for matching adventure actions from the AI so we can remove them
    acregex_ai = re.compile(r'\n* *>(.|\n)*')
    # Pattern for matching actions in the HTML-escaped story so we can apply
    # colouring, etc (make sure to encase part to format in parentheses)
    acregex_ui = re.compile(r'^ *(&gt;.*)$', re.MULTILINE)
    # Pattern for matching comments to remove them before sending them to the
    # AI
    ...
    newlinemode = "ns"
    # If set will suppress any story text from being printed to the console
    # (will only be seen on the client web page)
    quiet = False
    ...
    # Whether or not we're in a Colab TPU instance or Kaggle TPU instance and
    # are going to use the TPU rather than the CPU
    use_colab_tpu = os.environ.get(
    "COLAB_TPU_ADDR",
    "") != "" or os.environ.get(
        "TPU_NAME",
         "") != ""
...
class Send_to_socketio(object):
    def write(self, bar):
        ...
        try:
            gui_msg = bar.replace(
    f"{colors.PURPLE}INIT{colors.END}       | ",
    "").replace(
        " ",
         "&nbsp;")
            emit(
    'from_server', {
        'cmd': 'model_load_status', 'data': gui_msg}, broadcast=True)
        except BaseException:
            pass
...
def new_make_min_max_attributes(validators, min_attr, max_attr) -> dict:
    # Patched apispec function that creates
    # "exclusiveMinimum"/"exclusiveMaximum" OpenAPI attributes insteaed of
    # "minimum"/"maximum" when using validators.Range or validators.Length
    # with min_inclusive=False or max_inclusive=False
    attributes = {}
    min_list = [
    ...
    getattr(
        validator,
        "min_inclusive",
         True) for validator in validators if validator.min is not None]
    max_inclusive_list = [
    getattr(
        validator,
        "max_inclusive",
         True) for validator in validators if validator.max is not None]
    if min_list:
        if min_attr == "minimum" and not min_inclusive_list[max(
            range(len(min_list)), key=min_list.__getitem__)]:
            min_attr = "exclusiveMinimum"
        attributes[min_attr] = max(min_list)
    if max_list:
        if min_attr == "maximum" and not max_inclusive_list[min(
            range(len(max_list)), key=max_list.__getitem__)]:
...
def api_catch_out_of_memory_errors(f):
    ...
    def decorated(*args, **kwargs):
        try:
            ...
            if any(s in traceback.format_exc().lower()
                   for s in ("out of memory", "not enough memory")):
                for line in reversed(traceback.format_exc().split("\n")):
                    if any(s in line.lower() for s in ("out of memory",
                           "not enough memory")) and line.count(":"):
                        line = line.split(":", 1)[1]
                        line = re.sub(r"\[.+?\] +data\.", "", line).strip()
                        raise KoboldOutOfMemoryError("KoboldAI ran out of memory: " + line, type="out_of_memory.gpu.cuda" if "cuda out of memory" in line.lower() else "out_of_memory.gpu.hip" if "hip out of memory" in line.lower(
                        ) else "out_of_memory.tpu.hbm" if "memory space hbm" in line.lower() else "out_of_memory.cpu.default_memory_allocator" if "defaultmemoryallocator" in line.lower() else "out_of_memory.unknown.unknown")
...
def api_schema_wrap(f):
    try:
        input_schema: Type[Schema] = next(
            iter(inspect.signature(f).parameters.values())).annotation
    except BaseException:
        HAS_SCHEMA = False
    else:
        HAS_SCHEMA = inspect.isclass(
            input_schema) and issubclass(input_schema, Schema)
...
def handler(e):
    if request.path != "/api" and not request.path.startswith("/api/"):
        return e
    resp = jsonify(detail={"msg": str(e),
     "type": "generic.error_" + str(e.code)})
...
def handler(e):
    if request.path != "/api" and not request.path.startswith("/api/"):
        return InternalServerError()
    return jsonify(detail={"type": "not_implemented",
                   "msg": str(e).strip()}), 501
...
class KoboldAPISpec(APISpec):
    class KoboldFlaskPlugin(FlaskPlugin):
        ...
        def path_helper(self, *args, **kwargs):
            return super().path_helper(
                *args, **kwargs)[len(self._kobold_api_spec._prefixes[0]):]

    def __init__(self, *args, title: str = "KoboldAI API", openapi_version: str = "3.0.3",
                 version: str = "1.0.0", prefixes: List[str] = None, **kwargs):
     ...
     **kwargs)
        for prefix in self._prefixes:
            app.route(
    prefix,
    endpoint="~KoboldAPISpec~" +
    prefix)(
        lambda: redirect(
            request.path +
             "/docs/"))
            app.route(prefix + "/", endpoint="~KoboldAPISpec~" +
                      prefix + "/")(lambda: redirect("docs/"))
            app.route(prefix + "/docs", endpoint="~KoboldAPISpec~" +
                      prefix + "/docs")(lambda: redirect("docs/"))
            app.route(
    prefix +
    "/docs/",
    endpoint="~KoboldAPISpec~" +
    ...
    "/docs/")(
        lambda: render_template(
            "swagger-ui.html",
            url=self._prefixes[0] +
             "/openapi.json"))
            app.route(prefix +
    "/openapi.json", endpoint="~KoboldAPISpec~" +
    prefix +
     "/openapi.json")(lambda: jsonify(self.to_dict()))
...
# If the model_name is specified, it returns what the settings file would
# be for that model
...
def get_config_filename(model_name=None):
    if model_name:
        ...
        logger.warning(
            f"Empty configfile name sent back. Defaulting to ReadOnly")
...
def sendModelSelection(menu="mainmenu", folder="./models"):
    # If we send one of the manual load options, send back the list of model
    # directories, otherwise send the menu
    if menu in ('NeoCustom', 'GPT2Custom'):
        ...
        if os.path.abspath("{}/models".format(os.getcwd())
...
def get_folder_path_info(base):
    if base == 'This PC':
        ...
        paths = [["{}:\\".format(chr(i)), "{}:\\".format(chr(i))] for i in range(
            65, 91) if os.path.exists("{}:".format(chr(i)))]
    else:
        ...
        if len(breadcrumbs) == 1:
            breadcrumbs = [["{}:\\".format(chr(i)), "{}:\\".format(chr(i))] for i in range(
                65, 91) if os.path.exists("{}:".format(chr(i)))]
        else:
            if len([["{}:\\".format(chr(i)), "{}:\\".format(chr(i))]
                   for i in range(65, 91) if os.path.exists("{}:".format(chr(i)))]) > 0:
    ...
    # Paths/breadcrumbs is a list of lists, where the first element in the
    # sublist is the full path and the second is the folder name
...
def getModelSelection(modellist):
    ...
    while (vars.model == ''):
        ...
        if (modelsel.isnumeric() and int(modelsel) >
            0 and int(modelsel) <= len(modellist)):
            vars.model = modellist[int(modelsel) - 1][1]
        else:
            print(
    "{0}Please enter a valid selection.{1}".format(
    ...
    try:
        ...
        # If custom model was selected, get the filesystem location and store
        # it
    ...
    getcwd() + "/models", "Select Model Folder")
            ...
            if (modpath):
                ...
                print(
    "{0}Model select cancelled!{1}".format(
        colors.RED, colors.END))
                print(
    "{0}Select an AI model to continue:{1}\n".format(
        colors.CYAN, colors.END))
                getModelSelection(mainmenu)
...
def getmodelname():
    ...
    if (vars.model in ("NeoCustom", "GPT2Custom",
        "TPUMeshTransformerGPTJ", "TPUMeshTransformerGPTNeoX")):
...
def device_list(n_layers, primary=None, selected=None):
    ...
    gpu_blocks = breakmodel.gpu_blocks + \
        (device_count - len(breakmodel.gpu_blocks)) * [0]
...
def device_config(config):
    ...
    if args.cpu:
        ...
        try:
            if (not args.breakmodel_gpulayers):
                ...
                breakmodel.gpu_blocks = list(
                    map(int, args.breakmodel_gpulayers.split(',')))
        ...
        breakmodel.gpu_blocks = [n_layers -
     max(0, min(n_layers, args.breakmodel_layers))]
        n_layers -= sum(breakmodel.gpu_blocks)
    ...
    colors.CYAN +
     "\nPlease select one of your GPUs to be your primary GPU.")
            ...
            while (True):
                ...
                if (primaryselect.isnumeric() and 0 <=
                    int(primaryselect) < device_count):
    ...
    primary=breakmodel.primary_device,
     selected=i)
            ...
            while (True):
                ...
                if ((layerselect.isnumeric() or layerselect.strip() == '-1')
    ...
    if (len(breakmodel.gpu_blocks)
        and breakmodel.gpu_blocks[-1] in (-1, utils.num_layers(config))):
...
def move_model_to_devices(model):
    ...
    if (utils.HAS_ACCELERATE):
        ...
        for name in utils.layers_module_names:
            ...
            device = (
    "disk" if layer < disk_blocks else "cpu") if layer < ram_blocks else bisect.bisect_right(
        cumulative_gpu_blocks, layer - ram_blocks)
            device_map[name] = device
        for name in utils.get_missing_module_names(
            model, list(device_map.keys())):
...
def loadmodelsettings():
    ...
    if vars.model_type == "opt" or vars.model_type == "bloom":
        # Handle </s> but don't convert newlines if using Fairseq models that
        # have newlines trained in them
        vars.newlinemode = "ns"
    vars.modelconfig = js
...
def processsettings(js):
    ...
    if ("corescript" in js and type(js["corescript"]) is str and all(q not in js["corescript"] for q in (
        "..", ":")) and all(js["corescript"][0] not in q for q in ("/", "\\"))):
...
def check_for_sp_change():
    ...
    'from_server', {
        'cmd': 'spstatitems', 'data': {
            vars.spfilename: vars.spmeta} if vars.allowsp and len(
                vars.spfilename) else {}}, namespace=None, broadcast=True)
...
def spRequest(filename):
    ...
    if (vars.use_colab_tpu or vars.model in (
        "TPUMeshTransformerGPTJ", "TPUMeshTransformerGPTNeoX")):
        rows = tensor.shape[0]
...
def general_startup(override_args=None):
    ...
    "--remote",
    action='store_true',
     help="Optimizes KoboldAI for Remote Play")
    parser.add_argument(
    "--noaimenu",
    action='store_true',
     help="Disables the ability to select the AI")
    parser.add_argument(
    "--ngrok",
    action='store_true',
     help="Optimizes KoboldAI for Remote Play using Ngrok")
    parser.add_argument(
    "--localtunnel",
    action='store_true',
     help="Optimizes KoboldAI for Remote Play using Localtunnel")
    parser.add_argument(
    "--host",
    action='store_true',
    ...
    "--port",
    type=int,
     help="Specify the port on which the application will be joinable")
    parser.add_argument(
    "--aria2_port",
    type=int,
     help="Specify the port on which aria2's RPC interface will be open if aria2 is installed (defaults to 6799)")
    parser.add_argument(
    "--model",
     help="Specify the Model Type to skip the Menu")
    parser.add_argument(
    "--path",
     help="Specify the Path for local models (For model NeoCustom or GPT2Custom)")
    parser.add_argument(
    "--apikey",
     help="Specify the API key to use for online services")
    parser.add_argument(
    "--req_model",
    type=str,
    action='append',
    required=False,
     help="Which models which we allow to generate for us during cluster mode. Can be specified multiple times.")
    parser.add_argument(
    "--revision",
     help="Specify the model revision for huggingface models (can be a git branch/tag name or a git commit hash)")
    parser.add_argument(
    "--cpu",
    action='store_true',
    ...
    type=str,
     help="If using a model that supports hybrid generation, this is a comma-separated list that specifies how many layers to put on each GPU device. For example to put 8 layers on device 0, 9 layers on device 1 and 11 layers on device 2, use --breakmodel_gpulayers 8,9,11")
    parser.add_argument(
    "--breakmodel_disklayers",
    type=int,
     help="If using a model that supports hybrid generation, this is the number of layers to put in disk cache.")
    parser.add_argument(
    "--override_delete",
    action='store_true',
    ...
    parser.add_argument(
    "--configname",
     help="Force a fixed configuration name to aid with config management.")
    parser.add_argument(
    "--colab",
    action='store_true',
     help="Optimize for Google Colab.")
    parser.add_argument(
    "--nobreakmodel",
    action='store_true',
     help="Disables Breakmodel support completely.")
    parser.add_argument(
    "--unblock",
    action='store_true',
    default=False,
    ...
    action='store_true',
    default=False,
     help="Prevents KoboldAI from using aria2 to download huggingface models more efficiently, in case aria2 is causing you issues")
    parser.add_argument(
    "--lowmem",
    action='store_true',
    ...
    "--savemodel",
    action='store_true',
     help="Saves the model to the models folder even if --colab is used (Allows you to save models to Google Drive)")
    ...
    action='store_true',
    default=False,
     help="Disables the GUI and Socket.IO server while leaving the API server running.")
    parser.add_argument(
    '-v',
    '--verbosity',
    action='count',
    ...
    # Now let's look to see if we are going to force a load of a model from a
    ...
    getcwd() + "/models", "Select Model Folder")
        ...
        if args.path:
            ...
            vars.colaburl = args.path + "/request"
# ==================================================================#
# Load Model
# ==================================================================#
...
def tpumtjgetsofttokens():
    ...
    soft_tokens = np.arange(
        ...
        tpu_mtj_backend.params["n_vocab"] +
            tpu_mtj_backend.params["n_vocab_padding"] + vars.sp_length,
        dtype=np.uint32
    )
    return soft_tokens
    ...
    if model in ['Colab', 'API']:
        ...
        if layer_count is None:
            ...
            if model in ["NeoCustom", "GPT2Custom"]:
                filename = "settings/{}.breakmodel".format(
...
def get_layer_count(model, directory=""):
    if (model not in ["InferKit", "Colab", "API", "CLUSTER",
        "OAI", "GooseAI", "ReadOnly", "TPUMeshTransformerGPTJ"]):
        if (model == "GPT2Custom"):
            with open(os.path.join(directory, "config.json"), "r") as f:
                model_config = json.load(f)
        # Get the model_type from the config or assume a model type if it isn't
        # present
        else:
            if (directory):
                model = directory
            ...
            if (os.path.isdir(model.replace('/', '_'))):
                ...
                model_config = AutoConfig.from_pretrained(
                    "models/{}".format(model.replace('/', '_')), revision=args.revision, cache_dir="cache")
            elif (os.path.isdir(directory)):
                model_config = AutoConfig.from_pretrained(
    directory, revision=args.revision, cache_dir="cache")
            else:
                model_config = AutoConfig.from_pretrained(
    model, revision=args.revision, cache_dir="cache")
        try:
            if ((utils.HAS_ACCELERATE and model_config.model_type != 'gpt2') or model_config.model_type in (
                "gpt_neo", "gptj", "xglm", "opt")) and not vars.nobreakmodel:
...
def get_oai_models(key):
    ...
    if (req.status_code == 200):
        ...
        try:
            engines = [[en["id"], "{} ({})".format(
                en['id'], "Ready" if en["ready"] == True else "Not Ready")] for en in engines]
        except BaseException:
    ...
    'online_model': online_model},
     broadcast=True)
        ...
        # Something went wrong, print the message and quit since we can't
...
def get_cluster_models(msg):
    ...
    try:
        ...
        emit(
    'from_server', {
        'cmd': 'errmsg', 'data': "Provided KoboldAI Horde URL unreachable"})
        return
    if (not req.ok):
        # Something went wrong, print the message and quit since we can't
        # initialize an engine
        ...
        def write(self, bar):
            ...
            if bar != "":
                try:
                    ...
                    emit(
    'from_server', {
        'cmd': 'model_load_status', 'data': bar.replace(
            " ", "&nbsp;")}, broadcast=True)
...
def patch_transformers():
    ...
    if (transformers_version == "4.17.0.dev0"):
        try:
            ...
            @torch.no_grad()
            def new_forward(self, input_ids: torch.Tensor = None,
                            inputs_embeds: torch.Tensor = None, past_key_values_length: int = 0):
    ...
    self.embedding_dim,
     self.padding_idx)
                return self.weights.index_select(
                    0, position_ids.view(-1)).view(bsz, seq_len, -1).detach()
            XGLMSinusoidalPositionalEmbedding.forward = new_forward

    # Fix a bug in OPTForCausalLM where self.lm_head is the wrong size
    if (packaging.version.parse("4.19.0.dev0") <= packaging.version.parse(
        transformers_version) < packaging.version.parse("4.20.0")):
        try:
    ...
    def dynamic_processor_wrap(cls, field_name, var_name, cond=None):
        ...
        def new_call(self, *args, **kwargs):
            if (not isinstance(field_name, str)
                and isinstance(field_name, Iterable)):
                conds = []
    ...
    class LuaLogitsProcessor(LogitsProcessor):
        ...
        def __call__(self, input_ids: torch.LongTensor,
            ...
            for r, row in enumerate(scores_list):
                vars.lua_koboldbridge.logits[r +
                    1] = vars.lua_state.table(*row)
            vars.lua_koboldbridge.vocab_size = scores_shape[-1]

            execute_genmod()

            scores = torch.tensor(
                tuple(tuple(row.values())
                      for row in vars.lua_koboldbridge.logits.values()),
    ...
    def visualize_probabilities(scores: torch.FloatTensor) -> None:
        ...
        for token_id, score in sorted(
            enumerate(probs), key=lambda x: x[1], reverse=True)[:8]:
    ...
    class KoboldLogitsWarperList(LogitsProcessorList):
        def __init__(self, beams: int = 1, **kwargs):
            ...
            self.__warper_list.append(TopKLogitsWarper(
                top_k=1, min_tokens_to_keep=1 + (beams > 1)))
            self.__warper_list.append(TopALogitsWarper(
                top_a=0.5, min_tokens_to_keep=1 + (beams > 1)))
            self.__warper_list.append(TopPLogitsWarper(
                top_p=0.5, min_tokens_to_keep=1 + (beams > 1)))
            self.__warper_list.append(TailFreeLogitsWarper(
                tfs=0.5, min_tokens_to_keep=1 + (beams > 1)))
            self.__warper_list.append(TypicalLogitsWarper(
                typical=0.5, min_tokens_to_keep=1 + (beams > 1)))
            self.__warper_list.append(TemperatureLogitsWarper(temperature=0.5))
            self.__warper_list.append(
    AdvancedRepetitionPenaltyLogitsProcessor())

        def __call__(self, input_ids: torch.LongTensor,
                     scores: torch.FloatTensor, *args, **kwargs):
            sampler_order = vars.sampler_order[:]
            if len(
                sampler_order) < 7:  # Add repetition penalty at beginning if it's not present
                sampler_order = [6] + sampler_order
            for k in sampler_order:
                scores = self.__warper_list[k](
                    input_ids, scores, *args, **kwargs)
    ...
    class TokenStreamer(StoppingCriteria):
        ...
        def __call__(
            ...
            **kwargs,
            ...
            if vars.chatmode:
                return False
            tokenizer_text = utils.decodenewlines(
    ...
    class DynamicWorldInfoScanCriteria(StoppingCriteria):
        ...
        def __call__(
            ...
            **kwargs,
            ...
            if (not vars.standalone and vars.lua_koboldbridge.generated_cols and vars.generated_tkns !=
                vars.lua_koboldbridge.generated_cols):
                raise RuntimeError(
                    f"Inconsistency detected between KoboldAI Python and Lua backends ({vars.generated_tkns} != {vars.lua_koboldbridge.generated_cols})")
            ...
            for i in range(vars.numseqs):
                vars.lua_koboldbridge.generated[i +
    1][vars.generated_tkns] = int(input_ids[i, -
    1].item())
            ...
            for i, t in enumerate(tail):
                ...
                _, found = checkworldinfo(
    decoded, force_use_txt=True, actions=vars._actions)
...
def reset_model_settings():
    ...
    # If using a backend that supports Lua generation modifiers, how many
    # tokens have already been generated, otherwise 0
    vars.generated_tkns = 0
    vars.badwordsids = []
    # Whether or not the most recently loaded HF model was in fp32 format
    vars.fp32_model = False
    # Embedding dimension of your model (e.g. it's 4096 for GPT-J-6B and 2560
    # for GPT-Neo-2.7B)
    vars.modeldim = -1
...
def load_model(use_gpu=True, gpu_layers=None, disk_layers=None, initial_load=False,
               online_model="", use_breakmodel_args=False, breakmodel_args_default_to_cpu=False):
    global model
    global generator
    global torch
    global model_config
    ...
    if not use_breakmodel_args:
        ...
        if vars.model != 'ReadOnly':
            emit(
    'from_server', {
        'cmd': 'model_load_status', 'data': "Loading {}".format(
            vars.model)}, broadcast=True)
            # Have to add a sleep so the server will send the emit for some
            # reason
            time.sleep(0.1)
    ...
    if (vars.model not in ["InferKit", "Colab", "API", "CLUSTER", "OAI", "GooseAI",
        "ReadOnly", "TPUMeshTransformerGPTJ", "TPUMeshTransformerGPTNeoX"]):
        vars.allowsp = True
        ...
        # This code is not just a workaround for below, it is also used to make
        # the behavior consistent with other loading methods - Henk717
        ...
        if (os.path.isdir(vars.custmodpth.replace('/', '_'))):
            try:
                model_config = AutoConfig.from_pretrained(vars.custmodpth.replace(
                    '/', '_'), revision=args.revision, cache_dir="cache")
            ...
            try:
                model_config = AutoConfig.from_pretrained(
                    "models/{}".format(vars.custmodpth.replace('/', '_')), revision=args.revision, cache_dir="cache")
                vars.model_type = model_config.model_type
            except ValueError as e:
                vars.model_type = "not_found"
        else:
            try:
                model_config = AutoConfig.from_pretrained(
    vars.custmodpth, revision=args.revision, cache_dir="cache")
                vars.model_type = model_config.model_type
            except ValueError as e:
                vars.model_type = "not_found"
    ...
    if (not vars.use_colab_tpu and vars.model not in [
        "InferKit", "Colab", "API", "CLUSTER", "OAI", "GooseAI", "ReadOnly", "TPUMeshTransformerGPTJ", "TPUMeshTransformerGPTNeoX"]):
        loadmodelsettings()
        loadsettings()
        logger.init("GPU support", status="Searching")
        vars.hascuda = torch.cuda.is_available() and not args.cpu
        vars.bmsupported = (
    (utils.HAS_ACCELERATE and vars.model_type != 'gpt2') or vars.model_type in (
        "gpt_neo", "gptj", "xglm", "opt")) and not vars.nobreakmodel
        ...
        if (args.model and vars.bmsupported and not args.breakmodel_gpulayers and not args.breakmodel_layers and (
            not utils.HAS_ACCELERATE or not args.breakmodel_disklayers)):
            logger.warning(
                "Model launched without the --breakmodel_gpulayers argument, defaulting to GPU only mode.")
            vars.bmsupported = False
        if (not vars.bmsupported and (
            args.breakmodel_gpulayers is not None or args.breakmodel_layers is not None or args.breakmodel_disklayers is not None)):
            logger.warning(
...
if utils.HAS_ACCELERATE:
    ...
    if vars.usegpu:
        model = model.to(f"cuda:{vars.gpu_device}")  # Move model to the GPU
    else:
        model = model.to("cpu")  # If not using GPU, move to CPU
    ...
    disk_blocks = breakmodel.disk_blocks
    gpu_blocks = breakmodel.gpu_blocks
    ram_blocks = n_layers - sum(gpu_blocks)
    cumulative_gpu_blocks = tuple(itertools.accumulate(gpu_blocks))

    def lazy_load_callback(
        model_dict: Dict[str, Union[torch_lazy_loader.LazyTensor, torch.Tensor]], f, **_):
        ...
        # Correct placement of the decorator
        @functools.lru_cache(maxsize=None)
        def get_original_key(key):
            return max(
                (original_key for original_key in utils.module_names if original_key.endswith(key)), key=len)
        ...
        for key, value in model_dict.items():
            ...
            if isinstance(value, torch_lazy_loader.LazyTensor) and not any(
                ...
                # Determine device (GPU, CPU, or shared memory)
                if vars.hascuda and vars.usegpu:
                    device_map[key] = f"cuda:{vars.gpu_device}"  # Assign GPU
                else:
                    device_map[key] = "cpu"  # Assign CPU if no GPU is used
                if vars.breakmodel:
                    # Use breakmodel device if needed
                    device_map[key] = breakmodel.primary_device
            else:
                # Handle the model layers
                layer = int(max((n for n in utils.layers_module_names if original_key.startswith(
                    n)), key=len).rsplit(".", 1)[1])
                if vars.hascuda and vars.usegpu:
                    device_map[key] = f"cuda:{vars.gpu_device}"  # Assign GPU
                elif not vars.hascuda or not vars.breakmodel:
                    device_map[key] = "cpu"  # Assign CPU if not using GPU
                elif layer < breakmodel.disk_blocks:
                    device_map[key] = "disk"  # Use disk for some layers
                elif layer < breakmodel.ram_blocks:
                    device_map[key] = "cpu"  # Use CPU for RAM-based layers
                else:
                    device_map[key] = "shared"  # Shared layers across devices

        # Ensure the model layers are assigned to the right device
        for key, value in model_dict.items():
            if isinstance(value, torch_lazy_loader.LazyTensor):
                # Move each layer to the correct device
                value.to(device_map[key])
    ...
    utils.get_sharded_checkpoint_num_tensors(
         ...
         **utils.from_pretrained_kwargs))
                        ...
                        utils.bar = tqdm(
    total=num_tensors,
    desc=f"{colors.PURPLE}INIT{colors.END}       | Loading model tensors",
     file=Send_to_socketio())

                    with zipfile.ZipFile(f, "r") as z:
                        try:
                            last_storage_key = None
                            zipfolder = os.path.basename(
                                os.path.normpath(f)).split('.')[0]
                            f = None
                            current_offset = 0
                            able_to_pin_layers = True
                            if utils.num_shards is not None:
                                utils.current_shard += 1
                            for key in sorted(device_map.keys(), key=lambda k: (
                                model_dict[k].key, model_dict[k].seek_offset)):
                                storage_key = model_dict[key].key
                                if storage_key != last_storage_key or model_dict[
                                    key].seek_offset < current_offset:
                                    last_storage_key = storage_key
                                    if isinstance(f, zipfile.ZipExtFile):
                                        f.close()
                                    try:
                                        f = z.open(
                                            f"archive/data/{storage_key}")
                                    except BaseException:
                                        f = z.open(
                                            f"{zipfolder}/data/{storage_key}")
                                    current_offset = 0
                                if current_offset != model_dict[key].seek_offset:
                                    f.read(
    model_dict[key].seek_offset - current_offset)
                                    current_offset = model_dict[key].seek_offset
                                device = device_map[key]
                                size = functools.reduce(
    lambda x, y: x * y, model_dict[key].shape, 1)
                                dtype = model_dict[key].dtype
                                nbytes = size if dtype is torch.bool else size * \
                                ...
                                if convert_to_float16 and breakmodel.primary_device != "cpu" and vars.hascuda and (
                                    vars.breakmodel or vars.usegpu) and model_dict[key].dtype is torch.float32:
                                    model_dict[key] = model_dict[key].to(
                                        torch.float16)
                                if breakmodel.primary_device == "cpu" or (
                                    not vars.usegpu and not vars.breakmodel and model_dict[key].dtype is torch.float16):
                                    model_dict[key] = model_dict[key].to(
                                        torch.float32)
                                if device == "shared":
                                    model_dict[key] = model_dict[key].to(
                                    ...
                                    accelerate.utils.offload_weight(model_dict[key], get_original_key(
                                        key), "accelerate-disk-cache", index=utils.offload_index)
                                    model_dict[key] = model_dict[key].to(
                                        "meta")
                                else:
                                    model_dict[key] = model_dict[key].to(
                            ...
                            if utils.num_shards is None or utils.current_shard >= utils.num_shards:
                                if utils.offload_index:
                                    for name, tensor in utils.named_buffers:
                                        dtype = tensor.dtype
                                        if convert_to_float16 and breakmodel.primary_device != "cpu" and vars.hascuda and (
                                            vars.breakmodel or vars.usegpu):
                                            dtype = torch.float16
                                        if breakmodel.primary_device == "cpu" or (
                                            not vars.usegpu and not vars.breakmodel):
                                            dtype = torch.float32
                                        if name in model_dict and model_dict[name].dtype is not dtype:
                                            model_dict[name] = model_dict[name].to(
                                                dtype)
                                        if tensor.dtype is not dtype:
                                            tensor = tensor.to(dtype)
                                        if name not in utils.offload_index:
                                            accelerate.utils.offload_weight(
    tensor, name, "accelerate-disk-cache", index=utils.offload_index)
            ...
            def maybe_low_cpu_mem_usage() -> Dict[str, Any]:
                if (packaging.version.parse(transformers_version) < packaging.version.parse("4.11.0")):
                    logger.warning(
                        f"Please upgrade to transformers 4.11.0 for lower RAM usage. You have transformers {transformers_version}.")
                    return {}
    ...
    config_path, "config.json").replace(
        "\\", "//")
    ...
    vars.custmodpth, revision=args.revision, cache_dir="cache")
                model.save_pretrained(
                    "models/{}".format(vars.model.replace('/', '_')), max_shard_size="500MiB")
                tokenizer.save_pretrained(
                    "models/{}".format(vars.model.replace('/', '_')))
            ...
            # Use the Generic implementation
                ...
                # early so that it knows where to load the individual model
                # tensors
                ...
                if os.path.isdir(vars.model.replace('/', '_')):
                    import shutil
                    shutil.move(vars.model.replace('/', '_'),
                                "models/{}".format(vars.model.replace('/', '_')))
                if (vars.lazy_load):  # If we're using lazy loader, we need to figure out what the model's hidden layers are called
                    with torch_lazy_loader.use_lazy_torch_load(dematerialized_modules=True, use_accelerate_init_empty_weights=True):
                        try:
                            metamodel = AutoModelForCausalLM.from_config(
                                model_config)
                        except Exception as e:
                            metamodel = GPTNeoForCausalLM.from_config(
                                model_config)
                        utils.layers_module_names = utils.get_layers_module_names(
                            metamodel)
                        utils.module_names = list(
                            metamodel.state_dict().keys())
                        utils.named_buffers = list(
                            metamodel.named_buffers(recurse=True))
    ...
    vars.custmodpth, revision=args.revision, cache_dir="cache", use_fast=False)
                        except Exception as e:
                            try:
                                tokenizer = AutoTokenizer.from_pretrained(
    vars.custmodpth, revision=args.revision, cache_dir="cache")
    ...
    vars.custmodpth, revision=args.revision, cache_dir="cache")
                                    ...
                                    tokenizer = GPT2Tokenizer.from_pretrained(
                                        "gpt2", revision=args.revision, cache_dir="cache")
                        try:
        ...
        vars.model.replace(
            ...
            cache_dir="cache",
             use_fast=False)
                            ...
                            try:
                                tokenizer = AutoTokenizer.from_pretrained(
                                    "models/{}".format(vars.model.replace('/', '_')), revision=args.revision, cache_dir="cache")
                            except Exception as e:
                                try:
                                    tokenizer = GPT2Tokenizer.from_pretrained(
                                        "models/{}".format(vars.model.replace('/', '_')), revision=args.revision, cache_dir="cache")
                        ...
                        def new_rebuild_tensor(storage: Union[torch_lazy_loader.LazyTensor, torch.Storage], storage_offset, shape, stride):
                            ...
                            if (dtype is torch.float32 and len(shape) >= 2):
                                vars.fp32_model = True
                            return old_rebuild_tensor(
                                storage, storage_offset, shape, stride)
                        torch._utils._rebuild_tensor = new_rebuild_tensor

                        try:
                            tokenizer = AutoTokenizer.from_pretrained(
    vars.model, revision=args.revision, cache_dir="cache", use_fast=False)
                        except Exception as e:
                            try:
                                tokenizer = AutoTokenizer.from_pretrained(
                                    vars.model, revision=args.revision, cache_dir="cache")
                            except Exception as e:
                                try:
                                    tokenizer = GPT2Tokenizer.from_pretrained(
                                        vars.model, revision=args.revision, cache_dir="cache")
                        ...
                        if not args.colab or args.savemodel:
                            ...
                            if (vars.fp32_model and ("breakmodel" not in globals() or not breakmodel.disk_blocks)):  # Use save_pretrained to convert fp32 models to fp16, unless we are using disk cache because save_pretrained is not supported in that case
                                ...
                                legacy = packaging.version.parse(
                                    transformers_version) < packaging.version.parse("4.22.0.dev0")
                        ...
                        '_')),
                         transformers.configuration_utils.CONFIG_NAME))
                                if (utils.num_shards is None):
                                    # Save the pytorch_model.bin of an
                                    # unsharded model
                                    shutil.move(
    os.path.realpath(
        huggingface_hub.hf_hub_download(
                    ...
                    vars.model.replace(
                        ...
                        '_')),
                         transformers.modeling_utils.WEIGHTS_NAME))
                                    ...
                                    filenames = set(
    map_data["weight_map"].values())
                                    # Save the pytorch_model.bin.index.json of
                                    # a sharded model
                                    shutil.move(os.path.realpath(utils.from_pretrained_index_filename), os.path.join(
                                        "models/{}".format(vars.model.replace('/', '_')), transformers.modeling_utils.WEIGHTS_INDEX_NAME))
                                    # Then save the
                                    # pytorch_model-#####-of-#####.bin files
        ...
        huggingface_hub.hf_hub_download(
            ...
            os.path.join(
                ...
                if (vars.badwordsids is vars.badwordsids_default and vars.model_type not in ("gpt2", "gpt_neo", "gptj")):
                    vars.badwordsids = [[v] for k, v in tokenizer.get_vocab().items() if any(
                        c in str(k) for c in "<>[]") if vars.newlinemode != "s" or str(k) != "</s>"]
    ...
    "gpt2", revision=args.revision, cache_dir="cache")
        ...
        def tpumtjgenerate_stopping_callback(
            generated, n_generated, excluded_world_info) -> Tuple[List[set], bool, bool]:
            vars.generated_tkns += 1
    ...
    decoded, force_use_txt=True, actions=vars._actions)
        ...
        def tpumtjgenerate_compiling_callback() -> None:
            print(
    colors.GREEN +
    "TPU backend compilation triggered" +
     colors.END)
            vars.compiling = True
        ...
        if (vars.model in ("Colab", "API", "CLUSTER")):
            from transformers import GPT2Tokenizer
            tokenizer = GPT2Tokenizer.from_pretrained(
    "EleutherAI/gpt-neo-2.7B", revision=args.revision, cache_dir="cache")
            loadsettings()
        elif (vars.model == "OAI"):
            from transformers import GPT2Tokenizer
            tokenizer = GPT2Tokenizer.from_pretrained(
    ...
    "{0}Initializing Mesh Transformer JAX, please wait...{1}".format(
        colors.PURPLE, colors.END))
            if vars.model in ("TPUMeshTransformerGPTJ", "TPUMeshTransformerGPTNeoX") and (
                not vars.custmodpth or not os.path.isdir(vars.custmodpth)):
                raise FileNotFoundError(
                    f"The specified model path {repr(vars.custmodpth)} is not the path to a valid folder")
            import tpu_mtj_backend
            if (vars.model == "TPUMeshTransformerGPTNeoX"):
    ...
    vars.custmodpth,
    hf_checkpoint=vars.model not in (
        "TPUMeshTransformerGPTJ",
        "TPUMeshTransformerGPTNeoX") and vars.use_colab_tpu,
         **vars.modelconfig)
            vars.modeldim = int(
    tpu_mtj_backend.params.get(
        "d_embed",
...
def bridged_kwarg(name=None):
    def _bridged_kwarg(f: F):
        _bridged[name if name is not None else f.__name__[4:]
            if f.__name__[:4] == "lua_" else f.__name__] = f
        return f
...
def load_callback(filename, modulename):
    print(
    colors.GREEN +
    f"Loading Userscript [{modulename}] <{filename}>" +
     colors.END)
    ...
    modulenames,
     descriptions)
        ...
        if (vars.serverstarted):
            emit('from_server',
    {'cmd': 'errmsg',
    'data': 'Lua script error; please check console.'},
     broadcast=True)
            sendUSStatItems()
        logger.error('LUA ERROR: ' + str(e).replace("\033", ""))
        logger.warning(
...
def lua_encode(string):
    ...
    if ("tokenizer" not in globals()):
        ...
        tokenizer = GPT2Tokenizer.from_pretrained(
    "gpt2", revision=args.revision, cache_dir="cache")
    return tokenizer.encode(utils.encodenewlines(
        string), max_length=int(4e9), truncation=True)
...
def lua_set_attr(uid, k, v):
    ...
    print(
    colors.GREEN +
    f"{lua_log_format_name(vars.lua_koboldbridge.logging_name)} set {k} of world info entry {uid} to {v}" +
     colors.END)
...
def lua_folder_set_attr(uid, k, v):
    ...
    print(
    colors.GREEN +
    f"{lua_log_format_name(vars.lua_koboldbridge.logging_name)} set {k} of world info folder {uid} to {v}" +
     colors.END)
...
def lua_set_genamt(genamt):
    assert vars.lua_koboldbridge.userstate != "genmod" and type(
        genamt) in (int, float) and genamt >= 0
    print(
    colors.GREEN +
    f"{lua_log_format_name(vars.lua_koboldbridge.logging_name)} set genamt to {int(genamt)}" +
     colors.END)
...
def lua_set_numseqs(numseqs):
    ...
    print(
    colors.GREEN +
    f"{lua_log_format_name(vars.lua_koboldbridge.logging_name)} set numseqs to {int(numseqs)}" +
     colors.END)
...
def lua_set_setting(setting, v):
    ...
    actual_type is type(v) or (
        actual_type is int and type(v) is float))
    v = actual_type(v)
    print(
    colors.GREEN +
    f"{lua_log_format_name(vars.lua_koboldbridge.logging_name)} set {setting} to {v}" +
     colors.END)
...
def lua_set_chunk(k, v):
    ...
    if (len(v) == 0):
        print(
    colors.GREEN +
    f"{lua_log_format_name(vars.lua_koboldbridge.logging_name)} deleted story chunk {k}" +
     colors.END)
        chunk = int(k)
        ...
        if (k == 0):
            print(
    colors.GREEN +
    f"{lua_log_format_name(vars.lua_koboldbridge.logging_name)} edited prompt chunk" +
     colors.END)
        else:
            print(
    colors.GREEN +
    f"{lua_log_format_name(vars.lua_koboldbridge.logging_name)} edited story chunk {k}" +
     colors.END)
        chunk = int(k)
        if (chunk == 0):
            if (vars.lua_koboldbridge.userstate == "genmod"):
...
def lua_is_custommodel():
    return vars.model in ("GPT2Custom", "NeoCustom",
                          "TPUMeshTransformerGPTJ", "TPUMeshTransformerGPTNeoX")
...
def do_connect():
    ...
    emit('from_server', {'cmd': 'setanotetemplate',
         'data': vars.authornotetemplate})
    emit('from_server',
    {'cmd': 'connected',
    'smandelete': vars.smandelete,
    'smanrename': vars.smanrename,
    ...
    'from_server', {
        'cmd': 'spstatitems', 'data': {
            vars.spfilename: vars.spmeta} if vars.allowsp and len(
                vars.spfilename) else {}}, broadcast=True)
    ...
    emit('from_server', {'cmd': 'gamesaved',
         'data': vars.gamesaved}, broadcast=True)
...
def get_message(msg):
    ...
    if (msg['cmd'] == 'submit'):
        if (vars.mode == "play"):
            ...
            if (vars.chatmode):
                ...
                emit(
    'from_server', {
        'cmd': 'setchatname', 'data': vars.chatname})
    ...
    # EditMode Action (old)
        ...
        if (vars.mode == "play"):
            ...
            emit(
    'from_server', {
        'cmd': 'editmode', 'data': 'true'}, broadcast=True)
        elif (vars.mode == "edit"):
            vars.mode = "play"
            emit(
    'from_server', {
        'cmd': 'editmode', 'data': 'false'}, broadcast=True)
    # EditLine Action (old)
    ...
    # Author's Note depth update
        ...
        emit('from_server', {'cmd': 'setlabelanotedepth',
             'data': msg['data']}, broadcast=True)
        settingschanged()
        refresh_settings()
    ...
    'data': msg['data']},
     broadcast=True)
        ...
        emit('from_server', {'cmd': 'wiupdate', 'num': msg['num'], 'data': {
             field: vars.worldinfo[num][field] for field in fields}}, broadcast=True)
    ...
    'from_server', {
        ...
        emit(
    'from_server', {
        'cmd': 'buildsamplers', 'data': vars.sampler_order})
        ...
        if (not isinstance(sampler_order, list)):
            raise ValueError(
                f"Sampler order must be a list, but got a {type(sampler_order)}")
        if (not (sampler_order_min_length <= len(sampler_order) <= sampler_order_max_length)):
            raise ValueError(
                f"Sampler order must be a list of length greater than or equal to {sampler_order_min_length} and less than or equal to {sampler_order_max_length}, but got a list of length {len(sampler_order)}")
        if (not all(isinstance(e, int) for e in sampler_order)):
            raise ValueError(
                f"Sampler order must be a list of ints, but got a list with at least one non-int element")
        if (min(sampler_order) != 0 or max(sampler_order) != len(sampler_order) - 1 or len(set(sampler_order)) != len(sampler_order)):
            raise ValueError(
                f"Sampler order list of length {len(sampler_order)} must be a permutation of the first {len(sampler_order)} nonnegative integers")
        vars.sampler_order = sampler_order
        settingschanged()
        ...
        if not utils.HAS_ACCELERATE:
            msg['disk_layers'] = "0"
        if os.path.exists(
            "settings/" + vars.model_selected.replace('/', '_') + ".breakmodel"):
        ...
        load_model(
    use_gpu=msg['use_gpu'],
    gpu_layers=msg['gpu_layers'],
    disk_layers=msg['disk_layers'],
     online_model=msg['online_model'])
    elif (msg['cmd'] == 'show_model'):
        logger.info(f"Model Name: {getmodelname()}")
        emit('from_server', {'cmd': 'show_model_name',
             'data': getmodelname()}, broadcast=True)
        ...
        # so if that's missing we need to run the menu to show the model
        # folders in the models folder
        if msg['data'] in (
            'NeoCustom', 'GPT2Custom') and 'path' not in msg and 'path_modelname' not in msg:
            ...
            if check_if_dir_is_model(msg['path_modelname']):
                ...
                try:
                    ...
                    emit(
    'from_server', {
        'cmd': 'errmsg', 'data': "The model entered doesn't exist."})
        ...
        if "{}/models".format(os.getcwd()) in os.path.abspath(
            msg['data']) or "{}\\models".format(os.getcwd()) in os.path.abspath(msg['data']):
            if check_if_dir_is_model(msg['data']):
                ...
                logger.error(
                    f"Someone attempted to delete {msg['data']} but this is not a valid model")
        else:
            logger.critical(
                f"Someone maliciously attempted to delete {msg['data']}. The attempt has been blocked.")
        ...
        vars.debug = msg['data']
        emit('from_server', {'cmd': 'set_debug',
             'data': msg['data']}, broadcast=True)
        if vars.debug:
            send_debug()
        ...
        if "tokenizer" not in globals():
            ...
            emit(
                'from_server',
                {'cmd': 'showfieldbudget', 'data': {
                    "length": None, "max": None, "field": field}},
            )
            return
        ...
        if not unencoded:
            ...
            emit(
                'from_server',
                {'cmd': 'showfieldbudget', 'data': {
                    "length": 0, "max": max_tokens, "field": field}},
                broadcast=True
            )
        else:
            if field == "anoteinput":
                unencoded = buildauthorsnote(
    unencoded, msg["data"]["anotetemplate"])
            tokens_length = len(tokenizer.encode(unencoded))

            emit(
                'from_server',
                {'cmd': 'showfieldbudget', 'data': {
                    "length": tokens_length, "max": max_tokens, "field": field}},
                broadcast=True
            )
...
def sendUSStatItems():
    ...
    {'cmd': 'usstatitems',
    'data': loaded,
    'flash': last_userscripts != vars.last_userscripts},
     broadcast=True)
...
def setStartState():
    ...
    'data': txt},
     broadcast=True)
    emit('from_server', {'cmd': 'setgamestate',
         'data': 'start'}, broadcast=True)
...
def check_for_backend_compilation():
    ...
    {
        'cmd': 'warnmsg',
        'data': 'Compiling TPU backend&mdash;this usually takes 1&ndash;2 minutes...'},
         broadcast=True)
            break
    vars.checking = False
...
def actionsubmit(data, actionmode=0, force_submit=False, force_prompt_gen=False, disable_recentrng=False, no_generate=False, ignore_aibusy=False):
    ...
    while (True):
        ...
        if(vars.model in ["API", "CLUSTER"]):
            ...
            if vars.model == "API":
                ...
                # If the player has requested one or more models, we use the
                # first one for the tokenizer
            ...
            if tokenizer_id != vars.api_tokenizer_id:
                try:
                    if (os.path.isdir(tokenizer_id)):
                        try:
                            tokenizer = AutoTokenizer.from_pretrained(
    tokenizer_id, revision=args.revision, cache_dir="cache")
                        except BaseException:
                            tokenizer = AutoTokenizer.from_pretrained(
    tokenizer_id, revision=args.revision, cache_dir="cache", use_fast=False)
                    elif (os.path.isdir("models/{}".format(tokenizer_id.replace('/', '_')))):
                        try:
                            tokenizer = AutoTokenizer.from_pretrained(
                                "models/{}".format(tokenizer_id.replace('/', '_')), revision=args.revision, cache_dir="cache")
                        except BaseException:
                            tokenizer = AutoTokenizer.from_pretrained(
    "models/{}".format(
        tokenizer_id.replace(
            '/',
            '_')),
            revision=args.revision,
            cache_dir="cache",
    ...
    tokenizer_id, revision=args.revision, cache_dir="cache", use_fast=False)
        ...
        if (not vars.gamestarted):
            ...
            if (not no_generate):
                execute_inmod()
            # Remove trailing whitespace, excluding newlines
    ...
    {'cmd': 'updatescreen',
    'gamestarted': False,
    'data': 'Please wait, generating story...'},
     broadcast=True)
                ...
                emit(
    'from_server', {
        'cmd': 'scrolldown', 'data': ''}, broadcast=True)
                break
            else:
                # Save this first action as the prompt
                ...
                if (len(genout) == 1):
                    ...
                    if (len(vars.actions) > 0):
                        emit(
    'from_server', {
        'cmd': 'texteffect', 'data': vars.actions.get_last_key() + 1}, broadcast=True)
                ...
                if (len(vars.prompt.strip()) == 0):
                    ...
                    if vars.actions.get_last_key() not in vars.actions_metadata:
                        vars.actions_metadata[vars.actions.get_last_key()] = {
                                                                        "Selected Text": data, "Alternative Text": []}
                    else:
...
def apiactionsubmit_generate(txt, minimum, maximum):
    ...
    if not vars.quiet:
        ...
        logger.prompt(utils.decodenewlines(tokenizer.decode(
            txt)).encode("unicode_escape").decode("utf-8"))
    ...
    _genout, already_generated = tpool.execute(
        _generate, txt, minimum, maximum, set())

    genout = [applyoutputformatting(utils.decodenewlines(
        tokenizer.decode(tokens[-already_generated:]))) for tokens in _genout]
...
def apiactionsubmit_tpumtjgenerate(txt, minimum, maximum):
    ...
    applyoutputformatting(
        utils.decodenewlines(
            tokenizer.decode(txt))) for txt in genout]
...
def apiactionsubmit(data, use_memory=False, use_world_info=False, use_story=False, use_authors_note=False):
    if (vars.model == "Colab"):
        raise NotImplementedError(
            "API generation is not supported in old Colab API mode.")
    elif (vars.model == "API"):
        raise NotImplementedError(
            "API generation is not supported in API mode.")
    elif (vars.model == "CLUSTER"):
        raise NotImplementedError(
            "API generation is not supported in API mode.")
    elif (vars.model == "OAI"):
        raise NotImplementedError(
            "API generation is not supported in OpenAI/GooseAI mode.")
    elif (vars.model == "ReadOnly"):
        raise NotImplementedError(
            "API generation is not supported in read-only mode; please load a model and then try again.")
    ...
    mem_tokens = []
    wi_tokens = []
    def story_budget(): return vars.max_length - vars.sp_length - vars.genamt - len(tokenizer._koboldai_header) - len(story_tokens) - len(mem_tokens) - len(wi_tokens)
    def budget(): return story_budget() + MIN_STORY_TOKENS
    if budget() < 0:
    ...
    if use_world_info:
        world_info, _ = checkworldinfo(
    data, force_use_txt=True, scan_story=use_story)
        wi_tokens = tokenizer.encode(
            utils.encodenewlines(world_info))[-budget():]
    if use_story:
        if vars.useprompt:
            story_tokens = tokenizer.encode(
                utils.encodenewlines(vars.prompt))[-budget():]
    story_tokens = tokenizer.encode(utils.encodenewlines(
        data))[-story_budget():] + story_tokens
    if use_story:
        for i, action in enumerate(reversed(vars.actions.values())):
            if story_budget() <= 0:
                ...
                break
            story_tokens = tokenizer.encode(utils.encodenewlines(
                action))[-story_budget():] + story_tokens
            if i == vars.andepth - 1:
                story_tokens = tokenizer.encode(utils.encodenewlines(
                    anotetxt))[-story_budget():] + story_tokens
        if not vars.useprompt:
            story_tokens = tokenizer.encode(utils.encodenewlines(
                vars.prompt))[-budget():] + story_tokens
...
def actionretry(data):
    if (vars.noai):
        emit(
    'from_server', {
        'cmd': 'errmsg', 'data': "Retry function unavailable in Read Only mode."})
        return
    ...
    if actionback():
        ...
        emit('from_server',
    {'cmd': 'errmsg',
     'data': "Please enable \"Always Add Prompt\" to retry with your prompt."})
...
def actionback():
    ...
    if (len(vars.genseqs) == 0 and len(vars.actions) > 0):
        # We are going to move the selected text to alternative text in the
        # actions_metadata variable so we can redo this action
        ...
        emit(
    'from_server', {
        'cmd': 'errmsg', 'data': "Cannot delete the prompt."})
        success = False
    else:
        vars.genseqs = []
        success = True
...
def actionredo():
    ...
    flash=True,
     ignore_formatting=True)
                ...
                emit(
    'from_server', {
        'cmd': 'genseqs', 'data': genout}, broadcast=True)
    else:
        emit('from_server',
    {'cmd': 'popuperror',
    'data': "There's nothing to undo"},
     broadcast=True)
    send_debug()
...
def calcsubmitbudget(actionlen, winfo, mem, anotetxt, actions, submission=None, budget_deduction=0):
    ...
    utils.encodenewlines(
        vars.comregex_ai.sub(
            '',
            vars.prompt)),
            max_length=int(2e9),
             truncation=True)
    ...
    memtokens = tokenizer.encode(
    utils.encodenewlines(mem),
    max_length=int(2e9),
     truncation=True)
    lnmem = len(memtokens)
    if (lnmem > vars.max_length - lnheader - lnsp - vars.genamt - budget_deduction):
        raise OverflowError(
            "The memory in your story is too long. Please either write a shorter memory text or increase the Max Tokens setting. If you are using a soft prompt, additionally consider using a smaller soft prompt.")
    ...
    if (anotetxt != ""):
        anotetkns = tokenizer.encode(
    utils.encodenewlines(anotetxt),
    max_length=int(2e9),
     truncation=True)
        lnanote = len(anotetkns)
        if (lnmem + lnwi + lnanote > vars.max_length - lnheader - lnsp - vars.genamt - budget_deduction):
            raise OverflowError(
                "The author's note in your story is too long. Please either write a shorter author's note or increase the Max Tokens setting. If you are using a soft prompt, additionally consider using a smaller soft prompt.")

    if (vars.useprompt):
        budget = vars.max_length - lnheader - lnsp - lnprompt - \
            lnmem - lnanote - lnwi - vars.genamt - budget_deduction
    else:
        budget = vars.max_length - lnheader - lnsp - lnmem - \
            lnanote - lnwi - vars.genamt - budget_deduction
    ...
    if (actionlen == 0):
        # First/Prompt action
        tokens = (
    tokenizer._koboldai_header if vars.model not in (
        "Colab",
        "API",
        "CLUSTER",
         "OAI") else []) + memtokens + witokens + anotetkns + prompttkns
        assert len(tokens) <= vars.max_length - \
                   lnsp - vars.genamt - budget_deduction
        ln = len(tokens) + lnsp
        ...
        for key in reversed(actions):
            ...
            if (budget <= 0):
                break
            acttkns = tokenizer.encode(
    utils.encodenewlines(chunk),
    max_length=int(2e9),
     truncation=True)
            tknlen = len(acttkns)
            if (tknlen < budget):
    ...
    tokenizer._koboldai_header if vars.model not in (
        ...
        "API",
        "CLUSTER",
         "OAI") else []) + memtokens + witokens + anotetkns + prompttkns + tokens
            else:
                tokens = (
    tokenizer._koboldai_header if vars.model not in (
        "Colab",
        "API",
        "CLUSTER",
         "OAI") else []) + memtokens + witokens + prompttkns + tokens
        else:
...
def calcsubmit(txt):
    ...
    if (vars.model != "InferKit"):
        subtxt, min, max = calcsubmitbudget(
    actionlen, winfo, mem, anotetxt, vars.actions, submission=txt)
        if (actionlen == 0):
            if (not vars.use_colab_tpu and vars.model not in ["Colab", "API", "CLUSTER", "OAI", "TPUMeshTransformerGPTJ", "TPUMeshTransformerGPTNeoX"]):
                ...
                sendtocolab(
    utils.decodenewlines(
        tokenizer.decode(subtxt)), min, max)
            elif (vars.model == "API"):
                sendtoapi(
    utils.decodenewlines(
        tokenizer.decode(subtxt)), min, max)
    ...
    # For InferKit web API
        ...
        if (vars.useprompt):
            budget = vars.ikmax - \
                len(vars.comregex_ai.sub('', vars.prompt)) - len(anotetxt) - len(mem) - len(winfo) - 1
        else:
...
def _generate(txt, minimum, maximum, found_entries):
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            if (vars.lua_koboldbridge.generated_cols and vars.generated_tkns != vars.lua_koboldbridge.generated_cols):
                raise RuntimeError(
                    "Inconsistency detected between KoboldAI Python and Lua backends")
            ...
            for i in range(vars.numseqs):
                txt = utils.decodenewlines(
                    tokenizer.decode(genout[i, -already_generated:]))
                winfo, mem, anotetxt, _found_entries = calcsubmitbudgetheader(
                    txt, force_use_txt=True, actions=vars._actions)
                found_entries[i].update(_found_entries)
                txt, _, _ = calcsubmitbudget(
                    len(vars._actions), winfo, mem, anotetxt, vars._actions, submission=txt)
                encoded.append(
    torch.tensor(
        txt,
        dtype=torch.long,
    ...
    tuple(
        torch.nn.functional.pad(
            ...
            len(e),
            0),
             value=model.config.pad_token_id or model.config.eos_token_id) for e in encoded))
            genout = torch.cat(
                (
                    encoded,
            ...
            if (vars.sp is not None):
                ...
                genout = torch.cat(
                    (soft_tokens.tile(vars.numseqs, 1), genout), dim=-1)
            assert genout.shape[-1] + vars.genamt - \
                already_generated <= vars.max_length
...
def generate(txt, minimum, maximum, found_entries=None):
    ...
    _generate, txt, minimum, maximum, found_entries)
    except Exception as e:
    ...
    'data': 'Lua script error; please check console.'},
     broadcast=True)
            ...
            emit('from_server',
    {'cmd': 'errmsg',
    'data': 'Error occurred during generator call; please check console.'},
     broadcast=True)
    ...
    if (vars.lua_koboldbridge.regeneration_required):
        ...
        genout = [{"generated_text": utils.decodenewlines(
            tokenizer.decode(tokens[-already_generated:]))} for tokens in genout]
...
def genresult(genout, flash=True, ignore_formatting=False):
    ...
    if (len(vars.prompt.strip()) == 0):
        ...
        if vars.actions.get_last_key() not in vars.actions_metadata:
            vars.actions_metadata[vars.actions.get_last_key()] = {
                                                            'Selected Text': genout, 'Alternative Text': []}
        else:
            vars.actions_metadata[vars.actions.get_last_key(
            )]['Selected Text'] = genout
    update_story_chunk('last')
    if (flash):
        emit('from_server', {'cmd': 'texteffect', 'data': vars.actions.get_last_key(
        ) + 1 if len(vars.actions) else 0}, broadcast=True)
    send_debug()
...
def genselect(genout):
    ...
    for result in genout:
        ...
        result["generated_text"] = applyoutputformatting(
            result["generated_text"])
        if not vars.quiet:
            logger.info(f"Generation Result {i}")
            logger.generation(result["generated_text"].encode(
                "unicode_escape").decode("utf-8"))
        i += 1

    # Add the options to the actions metadata
    # If we've already generated text for this action but haven't selected one
    # we'll want to kill all non-pinned, non-previous selection, and
    # non-edited options then add the new ones
    if vars.actions.get_next_id() in vars.actions_metadata:
        if (vars.actions_metadata[vars.actions.get_next_id()]
            ['Selected Text'] == ""):
            ...
            vars.actions_metadata[vars.actions.get_next_id()] = {'Selected Text': '', 'Alternative Text': [
                                                           {"Text": text["generated_text"], "Pinned": False, "Previous Selection": False, "Edited": False} for text in genout]}
    else:
        vars.actions_metadata[vars.actions.get_next_id()] = {'Selected Text': '', 'Alternative Text': [
                                                       {"Text": text["generated_text"], "Pinned": False, "Previous Selection": False, "Edited": False} for text in genout]}
...
def pinsequence(n):
    if n.isnumeric():
        ...
        if text in [item['Text']
            for item in vars.actions_metadata[vars.actions.get_next_id()]['Alternative Text']]:
            alternatives = vars.actions_metadata[vars.actions.get_next_id(
            )]['Alternative Text']
            for i in range(len(alternatives)):
                if alternatives[i]['Text'] == text:
                    alternatives[i]['Pinned'] = not alternatives[i]['Pinned']
                    break
            vars.actions_metadata[vars.actions.get_next_id(
            )]['Alternative Text'] = alternatives
    send_debug()
...
def sendtoapi(txt, min, max):
    ...
    "{0}{1}{2}".format(
        ...
        json.dumps(
            ...
            emit(
    'from_server', {
        'cmd': 'errmsg', 'data': errmsg}, broadcast=True)
            set_aibusy(0)
            return
...
def sendtocluster(txt, min, max):
    ...
    while not finished:
        try:
            req = requests.get(vars.colaburl[:-
    8] +
    "/api/v2/generate/text/status/" +
     request_id, headers=cluster_agent_headers)
        except requests.exceptions.ConnectionError:
            errmsg = f"Horde unavailable. Please try again later"
            logger.error(errmsg)
            emit(
    ...
    if len(generations) == 1:
        ...
        emit('from_server', {'cmd': 'warnmsg',
             'data': warnmsg}, broadcast=True)
...
def tpumtjgenerate(txt, minimum, maximum, found_entries=None):
    ...
    copy_current_request_context(check_for_backend_compilation))
    ...
    tuple(
        np.pad(
            ...
            len(e),
            0),
             constant_values=tpu_mtj_backend.pad_token_id) for e in encoded))
...
def getnewcontent(txt):
    ...
    ctxtokens = tokenizer.encode(
    utils.encodenewlines(
        vars.lastctx),
        max_length=int(2e9),
         truncation=True)
    txttokens = tokenizer.encode(
    utils.encodenewlines(txt),
    max_length=int(2e9),
     truncation=True)
...
def refresh_story():
    ...
    vars.comregex_ui.sub(
        lambda m: '\n'.join(
            '<comment>' +
            l +
            '</comment>' for l in m.group().split('\n')),
            html.escape(
                vars.prompt)),
                 '</chunk>']
    ...
    emit(
    'from_server', {
        'cmd': 'updatescreen', 'gamestarted': vars.gamestarted, 'data': formatforhtml(
            ''.join(text_parts))}, broadcast=True)
...
def update_story_chunk(idx: Union[int, str]):
    ...
    emit(
    'from_server', {
        'cmd': 'updatechunk', 'data': {
            'index': idx, 'html': chunk_text}}, broadcast=True)
...
def refresh_settings():
    ...
    if (vars.model != "InferKit"):
        emit('from_server', {'cmd': 'updatetemp',
             'data': vars.temp}, broadcast=True)
        emit('from_server', {'cmd': 'updatetopp',
             'data': vars.top_p}, broadcast=True)
        emit('from_server', {'cmd': 'updatetopk',
             'data': vars.top_k}, broadcast=True)
        emit('from_server', {'cmd': 'updatetfs',
             'data': vars.tfs}, broadcast=True)
        emit('from_server', {'cmd': 'updatetypical',
             'data': vars.typical}, broadcast=True)
        emit('from_server', {'cmd': 'updatetopa',
             'data': vars.top_a}, broadcast=True)
        emit('from_server', {'cmd': 'updatereppen',
             'data': vars.rep_pen}, broadcast=True)
        emit('from_server', {'cmd': 'updatereppenslope',
             'data': vars.rep_pen_slope}, broadcast=True)
        emit('from_server', {'cmd': 'updatereppenrange',
             'data': vars.rep_pen_range}, broadcast=True)
        emit('from_server', {'cmd': 'updateoutlen',
             'data': vars.genamt}, broadcast=True)
        emit('from_server', {'cmd': 'updatetknmax',
             'data': vars.max_length}, broadcast=True)
        emit('from_server', {'cmd': 'updatenumseq',
             'data': vars.numseqs}, broadcast=True)
    else:
        emit('from_server', {'cmd': 'updatetemp',
             'data': vars.temp}, broadcast=True)
        emit('from_server', {'cmd': 'updatetopp',
             'data': vars.top_p}, broadcast=True)
        emit('from_server', {'cmd': 'updateikgen',
             'data': vars.ikgen}, broadcast=True)

    emit('from_server', {'cmd': 'updateanotedepth',
         'data': vars.andepth}, broadcast=True)
    emit('from_server', {'cmd': 'updatewidepth',
         'data': vars.widepth}, broadcast=True)
    emit('from_server', {'cmd': 'updateuseprompt',
         'data': vars.useprompt}, broadcast=True)
    emit('from_server', {'cmd': 'updateadventure',
         'data': vars.adventure}, broadcast=True)
    emit('from_server', {'cmd': 'updatechatmode',
         'data': vars.chatmode}, broadcast=True)
    emit('from_server', {'cmd': 'updatedynamicscan',
         'data': vars.dynamicscan}, broadcast=True)
    emit('from_server', {'cmd': 'updateautosave',
         'data': vars.autosave}, broadcast=True)
    emit('from_server', {'cmd': 'updatenopromptgen',
         'data': vars.nopromptgen}, broadcast=True)
    emit('from_server', {'cmd': 'updaterngpersist',
         'data': vars.rngpersist}, broadcast=True)
    emit('from_server', {'cmd': 'updatenogenmod',
         'data': vars.nogenmod}, broadcast=True)
    emit('from_server', {'cmd': 'updatefulldeterminism',
         'data': vars.full_determinism}, broadcast=True)

    emit('from_server',
    {'cmd': 'updatefrmttriminc',
    'data': vars.formatoptns["frmttriminc"]},
     broadcast=True)
    emit('from_server',
    {'cmd': 'updatefrmtrmblln',
    'data': vars.formatoptns["frmtrmblln"]},
     broadcast=True)
    emit('from_server',
    {'cmd': 'updatefrmtrmspch',
    'data': vars.formatoptns["frmtrmspch"]},
     broadcast=True)
    emit('from_server',
    {'cmd': 'updatefrmtadsnsp',
    'data': vars.formatoptns["frmtadsnsp"]},
     broadcast=True)
    emit('from_server',
    {'cmd': 'updatesingleline',
    'data': vars.formatoptns["singleline"]},
     broadcast=True)
    emit('from_server', {'cmd': 'updateoutputstreaming',
         'data': vars.output_streaming}, broadcast=True)
    emit('from_server', {'cmd': 'updateshowbudget',
         'data': vars.show_budget}, broadcast=True)
    emit('from_server', {'cmd': 'updateshowprobs',
         'data': vars.show_probs}, broadcast=True)
...
def set_aibusy(state):
    ...
    if (state):
        ...
        emit(
    'from_server', {
        'cmd': 'setgamestate', 'data': 'wait'}, broadcast=True)
    else:
        vars.aibusy = False
        emit('from_server', {'cmd': 'setgamestate',
             'data': 'ready'}, broadcast=True)
...
def editsubmit(data):
    ...
    emit('from_server', {'cmd': 'texteffect',
         'data': vars.editln}, broadcast=True)
...
def inlinedelete(chunk):
    ...
    'from_server', {
        'cmd': 'errmsg', 'data': "Cannot delete the prompt."})
        emit('from_server', {'cmd': 'editmode',
             'data': 'false'}, broadcast=True)
    else:
        if(chunk -1 in vars.actions):
...
def togglememorymode():
    if (vars.mode == "play"):
        ...
        emit('from_server', {'cmd': 'setinputtext',
             'data': vars.memory}, broadcast=True)
        emit('from_server', {'cmd': 'setanote',
             'data': vars.authornote}, broadcast=True)
        emit('from_server', {'cmd': 'setanotetemplate',
             'data': vars.authornotetemplate}, broadcast=True)
    elif (vars.mode == "memory"):
        vars.mode = "play"
        emit('from_server', {'cmd': 'memmode',
             'data': 'false'}, broadcast=True)
...
def addwifolder():
    ...
    emit('from_server', {'cmd': 'addwifolder',
         'uid': uid, 'data': ob}, broadcast=True)
...
def sendwi():
    ...
    emit('from_server',
    {'cmd': 'wistart',
    'wifolders_d': vars.wifolders_d,
    'wifolders_l': vars.wifolders_l,
    'data': ''},
     broadcast=True)
    ...
    {'cmd': 'addwifolder',
    'uid': wi["folder"],
    'data': vars.wifolders_d[wi["folder"]] if wi["folder"] is not None else None},
     broadcast=True)
                last_folder = wi["folder"]
            ob = wi
            emit(
    'from_server', {
        'cmd': 'addwiitem', 'data': ob}, broadcast=True)
    ...
    if (uid in vars.worldinfo_u):
        ...
        if (vars.deletewi is not None):
            if (vars.worldinfo_u[vars.deletewi]["folder"] is not None):
                for i, e in enumerate(
                    vars.wifolders_u[vars.worldinfo_u[vars.deletewi]["folder"]]):
                    if (e is vars.worldinfo_u[vars.deletewi]):
                        vars.wifolders_u[vars.worldinfo_u[vars.deletewi]["folder"]].pop(
                            i)
            for i, e in enumerate(vars.worldinfo):
                if (e is vars.worldinfo_u[vars.deletewi]):
...
def deletewifolder(uid):
    ...
    vars.worldinfo = [
    wi for wi in vars.worldinfo if wi["folder"] != uid or wi["init"]]
    vars.worldinfo_i = [wi for wi in vars.worldinfo if wi["init"]]
...
def ikrequest(txt):
    ...
    if (req.status_code == 200):
        ...
        if vars.actions.get_last_key() in vars.actions_metadata:
            ...
            # 2. We've selected a chunk of text that is was presented previously
            alternatives = [
                item['Text'] for item in vars.actions_metadata[vars.actions.get_last_key()]["Alternative Text"]]
            if genout in alternatives:
                alternatives = [item for item in vars.actions_metadata[vars.actions.get_last_key(
                )]["Alternative Text"] if item['Text'] != genout]
                vars.actions_metadata[vars.actions.get_last_key(
                )]["Alternative Text"] = alternatives
            vars.actions_metadata[vars.actions.get_last_key(
            )]["Selected Text"] = genout
...
def deletesave(name):
    ...
    if (e is None):
        if (vars.smandelete):
            ...
            emit('from_server',
    {'cmd': 'popuperror',
     'data': "The server denied your request to delete this story"})
    else:
        print("{0}{1}{2}".format(colors.RED, str(e), colors.END))
        emit('from_server', {'cmd': 'popuperror', 'data': str(e)})
...
def savetofile():
    savpath = fileops.getsavepath(
    vars.savedir, "Save Story As", [
        ("Json", "*.json")])
    saveRequest(savpath)
...
def saveRequest(savpath, savepins=True):
    if (savpath):
        ...
        emit('from_server', {'cmd': 'setstoryname',
             'data': vars.laststory}, broadcast=True)
        setgamesaved(True)
        print(
    "{0}Story saved to {1}!{2}".format(
        colors.GREEN,
...
def getsplist():
    if (vars.allowsp):
        emit(
    'from_server', {
        'cmd': 'buildsp', 'data': fileops.getspfiles(
            vars.modeldim)})
...
def loadfromfile():
    loadpath = fileops.getloadpath(
    vars.savedir, "Select Story File", [
        ("Json", "*.json")])
...
def loadRequest(loadpath, filename=None):
    ...
    int(k): v for k, v in js.get(
        ...
        if "actions_metadata" in js:
            ...
            if isinstance(js["actions_metadata"], dict):
                ...
                for text in js['actions']:
                    vars.actions_metadata[i] = {
    'Selected Text': text, 'Alternative Text': []}
                    i += 1
        else:
            vars.actions_metadata = {}
            i = 0
    ...
    'Selected Text': text, 'Alternative Text': []}
        ...
        if (vars.gamestarted):
            for s in actions:
                if (len(s.strip()) == 0):
                    # If this action only contains whitespace, we merge it with
                    # the next action
                    footer += s
                    continue
                vars.actions.append(footer + s)
                footer = ""
                # If there is trailing whitespace at the end of an action, we
                # move that whitespace to the beginning of the next action
                ln = len(vars.actions[vars.actions.get_last_key()].rstrip())
                footer += vars.actions[vars.actions.get_last_key()][ln:]
                vars.actions[vars.actions.get_last_key(
                )] = vars.actions[vars.actions.get_last_key()][:ln]
        ...
        if ("worldinfo" in js):
            ...
            for wi in js["worldinfo"]:
                ...
                if (vars.worldinfo[-1]["folder"] is not None):
                    vars.wifolders_u[vars.worldinfo[-1]
                        ["folder"]].append(vars.worldinfo[-1])
                num += 1

        for uid in vars.wifolders_l + [None]:
            vars.worldinfo.append({"key": "",
    "keysecondary": "",
    "content": "",
    "comment": "",
    "folder": uid,
    ...
    "constant": False,
     "uid": None})
        ...
        setgamesaved(True)
        sendwi()
        emit('from_server', {'cmd': 'setmemory',
             'data': vars.memory}, broadcast=True)
        emit('from_server', {'cmd': 'setanote',
             'data': vars.authornote}, broadcast=True)
        emit('from_server', {'cmd': 'setanotetemplate',
        ...
        emit('from_server', {'cmd': 'hidegenseqs', 'data': ''}, broadcast=True)
        print(
    "{0}Story loaded from {1}!{2}".format(
        colors.GREEN,
        filename,
         colors.END))
...
def importRequest():
    importpath = fileops.getloadpath(
    vars.savedir, "Select AID CAT File", [
        ("Json", "*.json")])
        ...
        if(ref["worldInfo"] is not None):
            if (len(ref["worldInfo"]) > 1):
                ...
                for wi in ref["worldInfo"]:
                    ...
                    while (True):
                        uid = int.from_bytes(
    os.urandom(4), "little", signed=True)
                        if (uid not in vars.worldinfo_u):
                            break
                    vars.worldinfo_u[uid] = vars.worldinfo[-1]
                    vars.worldinfo[-1]["uid"] = uid
...
def wiimportrequest():
    importpath = fileops.getloadpath(
    vars.savedir, "Select World Info File", [
        ("Json", "*.json")])
    if (importpath):
        file = open(importpath, "rb")
        js = json.load(file)
...
def final_startup():
    ...
    np.tile(
        np.uint32(
            (23403, 727, 20185)), (vars.numseqs, 1)),),
                kwargs={
                    "soft_embeddings": vars.sp,
                    "soft_tokens": soft_tokens,
                    "gen_len": 1,
    ...
    vars.seed = __import__("tpu_mtj_backend").get_rng_seed(
    ) if vars.use_colab_tpu else __import__("torch").initial_seed()
...
def send_debug():
    ...
    debug_info,
    repr(
        __import__("tpu_mtj_backend").get_rng_seed() if vars.use_colab_tpu else __import__("torch").initial_seed()),
         "specified by user in settings file" if vars.seed_specified else "randomly generated")
        except BaseException:
            pass
        try:
            debug_info = "{}Newline Mode: {}\n".format(
                debug_info, vars.newlinemode)
        except BaseException:
            pass
        try:
            debug_info = "{}Action Length: {}\n".format(
                debug_info, vars.actions.get_last_key())
        except BaseException:
            pass
        try:
            debug_info = "{}Actions Metadata Length: {}\n".format(
    debug_info, max(
        vars.actions_metadata) if len(
            vars.actions_metadata) > 0 else 0)
        except BaseException:
            pass
        try:
            debug_info = "{}Actions: {}\n".format(
                debug_info, [k for k in vars.actions])
        except BaseException:
            pass
        try:
            debug_info = "{}Actions Metadata: {}\n".format(
                debug_info, [k for k in vars.actions_metadata])
        except BaseException:
            pass
        try:
            debug_info = "{}Last Action: {}\n".format(
                debug_info, vars.actions[vars.actions.get_last_key()])
        except BaseException:
            pass
        try:
            debug_info = "{}Last Metadata: {}\n".format(
                debug_info, vars.actions_metadata[max(vars.actions_metadata)])
        except BaseException:
            pass

        emit('from_server', {'cmd': 'debug_info',
             'data': debug_info}, broadcast=True)
...
def popup_change_folder(data):
    ...
    if session['popup_jailed_dir'] is None:
        ...
        print(
    "User is trying to get at files in your server outside the jail. Blocked. Jailed Dir: {}  Requested Dir: {}".format(
        session['popup_jailed_dir'],
         data))
...
def popup_rename(data):
    ...
    if session['popup_jailed_dir'] is None:
        ...
        print(
    "User is trying to rename files in your server outside the jail. Blocked. Jailed Dir: {}  Requested Dir: {}".format(
        session['popup_jailed_dir'],
         data['file']))
...
def popup_delete(data):
    ...
    if session['popup_jailed_dir'] is None:
        ...
        print(
    "User is trying to delete files in your server outside the jail. Blocked. Jailed Dir: {}  Requested Dir: {}".format(
        session['popup_jailed_dir'],
         data))
...
def file_popup(popup_title, starting_folder, return_event, upload=True, jailed=True, folder_only=True, renameable=False, deleteable=False, editable=False, show_breadcrumbs=True, item_check=None, show_hidden=False):
    ...
    if jailed:
        session['popup_jailed_dir'] = os.path.abspath(
            starting_folder).replace("\\", "/")
    else:
    ...
    socketio.emit("load_popup",
    {"popup_title": popup_title,
    "call_back": return_event,
...
def get_files_folders(starting_folder):
    import stat
    session['current_folder'] = os.path.abspath(
        starting_folder).replace("\\", "/")
    ...
    if starting_folder == 'This PC':
        breadcrumbs = [['This PC', 'This PC']]
        items = [["{}:/".format(chr(i)), "{}:\\".format(chr(i))]
                                for i in range(65, 91) if os.path.exists("{}:".format(chr(i)))]
    else:
        ...
        for i in range(len(path.split("/"))):
            breadcrumbs.append(["/".join(path.split("/")[:i +1]),
                                path.split("/")[i]])
        if len(breadcrumbs) == 1:
            breadcrumbs = [["{}:/".format(chr(i)),
    "{}:\\".format(chr(i))] for i in range(65,
     91) if os.path.exists("{}:".format(chr(i)))]
        else:
            if len([["{}:/".format(chr(i)), "{}:\\".format(chr(i))]
                   for i in range(65, 91) if os.path.exists("{}:".format(chr(i)))]) > 0:
                breadcrumbs.insert(0, ['This PC', 'This PC'])
        ...
        if session['popup_jailed_dir'] is not None:
            ...
            breadcrumbs = breadcrumbs[len(
                session['popup_jailed_dir'].split("/")):]
        ...
        for item in os.listdir(base_path):
            ...
            if hasattr(os.stat(item_full_path), "st_file_attributes"):
                hidden = bool(
    os.stat(item_full_path).st_file_attributes & stat.FILE_ATTRIBUTE_HIDDEN)
            else:
                hidden = item[0] == "."
...
class BasicTextResultSchema(KoboldSchema):
    result: BasicTextResultInnerSchema = fields.Nested(
        BasicTextResultInnerSchema)
...
class ValidationErrorSchema(KoboldSchema):
    detail: Dict[str, List[str]] = fields.Dict(keys=fields.String(), values=fields.List(
...
class SamplerSettingsSchema(KoboldSchema):
    rep_pen: Optional[float] = fields.Float(validate=validate.Range(
        min=1), metadata={"description": "Base repetition penalty value."})
    rep_pen_range: Optional[int] = fields.Integer(validate=validate.Range(
        min=0), metadata={"description": "Repetition penalty range."})
    rep_pen_slope: Optional[float] = fields.Float(validate=validate.Range(
        min=0), metadata={"description": "Repetition penalty slope."})
    top_k: Optional[int] = fields.Integer(validate=validate.Range(
        min=0), metadata={"description": "Top-k sampling value."})
    top_a: Optional[float] = fields.Float(validate=validate.Range(
        min=0), metadata={"description": "Top-a sampling value."})
    top_p: Optional[float] = fields.Float(
    validate=validate.Range(
        min=0, max=1), metadata={
    ...
    validate=validate.Range(
        min=0, min_inclusive=False), metadata={
...
class GenerationInputSchema(SamplerSettingsSchema):
    ...
    sampler_order: Optional[List[int]] = fields.List(fields.Integer(), validate=[validate.Length(min=6), permutation_validator], metadata={
...
class GenerationOutputSchema(KoboldSchema):
    ...
    fields.Nested(GenerationResultSchema), required=True, metadata={
...
class StorySchema(KoboldSchema):
    ...
    fields.Nested(StoryChunkSchema), required=True, metadata={
...
class StoryNumsSchema(KoboldSchema):
    ...
    fields.Integer(), required=True, metadata={
...
class WorldInfoEntryResultSchema(KoboldSchema):
    result: WorldInfoEntrySchema = fields.Nested(
        WorldInfoEntrySchema, required=True)
...
class ModelSelectionSchema(KoboldSchema):
    ...
    required=True,
    validate=validate.Regexp(r"^(?!\s*NeoCustom)(?!\s*GPT2Custom)(?!\s*TPUMeshTransformerGPTJ)(?!\s*TPUMeshTransformerGPTNeoX)(?!\s*GooseAI)(?!\s*OAI)(?!\s*InferKit)(?!\s*Colab)(?!\s*API).*$"),
    metadata={
...
def _generate_text(body: GenerationInputSchema):
    ...
    if hasattr(body, "sampler_seed"):
        ...
        old_seed = vars.seed
        old_rng_state = tpu_mtj_backend.get_rng_state(
        ) if vars.use_colab_tpu else torch.get_rng_state()
        ...
        if body.sampler_seed in vars.rng_states:
            if vars.use_colab_tpu:
                tpu_mtj_backend.set_rng_state(
                    vars.rng_states[body.sampler_seed])
            else:
                torch.set_rng_state(vars.rng_states[body.sampler_seed])
        else:
            if vars.use_colab_tpu:
                tpu_mtj_backend.set_rng_state(
    tpu_mtj_backend.new_rng_state(
        body.sampler_seed))
            else:
                torch.manual_seed(body.sampler_seed)
        vars.rng_states[body.sampler_seed] = tpu_mtj_backend.get_rng_state(
        ) if vars.use_colab_tpu else torch.get_rng_state()
    ...
    for key, entry in mapping.items():
        ...
        if entry[2] == "input" and vars.disable_input_formatting and not hasattr(
            body, key):
            setattr(body, key, False)
        if entry[2] == "output" and vars.disable_output_formatting and not hasattr(
            body, key):
            setattr(body, key, False)
    ...
    use_world_info=body.use_world_info,
     use_authors_note=body.use_authors_note)
        ...
        for key in saved_settings:
            ...
            obj = {
    "vars": vars,
    "vars.formatoptns": vars.formatoptns}[
        entry[0]]
            if getattr(body, key, None) is not None:
                if entry[1].startswith("@"):
...
def put_model(body: ModelSelectionSchema):
    ...
    try:
        load_model(
    use_breakmodel_args=True,
     breakmodel_args_default_to_cpu=True)
    except Exception as e:
...
def get_story_end():
    ...
    )], "num": vars.actions.get_last_key() + 1}}
        ...
        for wi in vars.worldinfo_i:
            if wi["folder"] != last_folder:
                ...
                if wi["folder"] is not None:
                    folders.append(
                        {"uid": wi["folder"], "name": vars.wifolders_d[wi["folder"]]["name"], "entries": folder})
                last_folder = wi["folder"]
            (folder if wi["folder"] is not None else entries).append({k: v for k, v in wi.items(
            ) if k not in ("init", "folder", "num") and (wi["selective"] or k != "keysecondary")})
...
def get_world_info_uids_uid(uid: int):
    ...
    """
    return {
        "result": uid in vars.worldinfo_u and vars.worldinfo_u[uid]["init"]}
...
def get_world_info_folders():
    ...
    stablesortwi()
    vars.worldinfo_i = [wi for wi in vars.worldinfo if wi["init"]]
    return {"folders": [{"uid": folder, **{k: v for k, v in vars.wifolders_d[folder].items() if k != "collapsed"}}
                                                                                           for folder in vars.wifolders_l]}
...
def get_world_info_folders_none():
    ...
    for wi in reversed(vars.worldinfo_i):
        if wi["folder"] is not None:
            break
        entries.append({k: v for k, v in wi.items() if k not in (
            "init", "folder", "num") and (wi["selective"] or k != "keysecondary")})
...
def get_world_info_folders_none_uids_uid(uid: int):
    ...
    """
    return {
        "result": uid in vars.worldinfo_u and vars.worldinfo_u[uid]["folder"] is None and vars.worldinfo_u[uid]["init"]}
...
@api_schema_wrap
def get_world_info_folders_folder_uid_uids_entry_uid(
    folder_uid: int, entry_uid: int):
    """---
    get:
        ...
        200:
          ...
          content:
            application/json:
              schema: BasicBooleanSchema
    """
    return {"result": entry_uid in vars.worldinfo_u and vars.worldinfo_u[entry_uid]
        ["folder"] == folder_uid and vars.worldinfo_u[entry_uid]["init"]}
...
def get_world_info_uid(uid: int):
    ...
    return {k: v for k, v in wi.items() if k not in (
        "init", "folder", "num") and (wi["selective"] or k != "keysecondary")}
...
def post_world_info_folders_uid(body: EmptySchema, uid: int):
    ...
    emit('from_server',
    {'cmd': 'wiexpand',
    'data': vars.wifolders_u[uid][-1]["num"]},
     broadcast=True)
...
def create_config_endpoint(method="GET", schema="MemorySchema"):
    ...
    _name,
    _schema,
    _example_yaml_value) if method == "GET" else _make_f_put(
        globals()[schema],
        _obj,
        _var_name,
        _name,
...
class SoftPromptsListSchema(KoboldSchema):
    ...
    fields.Nested(SoftPromptSettingSchema), required=True, metadata={
...
def get_config_sampler_seed():
    ...
    """
    return {"value": __import__("tpu_mtj_backend").get_rng_seed(
    ) if vars.use_colab_tpu else __import__("torch").initial_seed()}
...
if __name__ == "__main__":
    ...
    if (vars.host):
        if (args.localtunnel):
            ...
            localtunnel = subprocess.Popen(
                [shutil.which('lt'), '-p', str(port), 'http'], stdout=subprocess.PIPE)
            attempts = 0
            while attempts < 10:
                try:
        ...
        if (args.localtunnel or args.ngrok or args.remote):
            with open('cloudflare.log', 'w') as cloudflarelog:
                cloudflarelog.write(
    "KoboldAI has finished loading and is available at the following link : " +
     cloudflare)
                logger.init_ok("Webserver", status="OK")
                logger.message(
                    f"KoboldAI has finished loading and is available at the following link: {cloudflare}")
        else:
            logger.init_ok("Webserver", status="OK")
            logger.message(
                f"Webserver has started, you can now connect to this machine at port: {port}")
        ...
        if args.unblock:
            ...
            logger.init_ok("Webserver", status="OK")
            logger.message(
                f"Webserver started! You may now connect with a browser at http://127.0.0.1:{port}")
            vars.serverstarted = True
    ...
    if vars.model == "" or vars.model is None:
        vars.model = "ReadOnly"
    load_model(initial_load=True)
    print("{0}\nServer started in WSGI mode!{1}".format(
        colors.GREEN, colors.END), flush=True)
